...
function uploadImage($form){
  ...
  request.open('post', 'Trkib_a/upload');
...
$(document).ready(function(){
  ...
  document.getElementById("sm3").style.color = "white";
  document.getElementById("sm3").style.fontWeight = "bold";
...
function sensus(id_barang,key){
  ...
  $.ajax({
    ...
    type : 'POST',
    data : {id_barang},
    url : 'Trkib_a/get_trkib_a_id_non',
    ...
    success : function(data){
      ...
      if (data[0].pengguna2=='' || data[0].pengguna2==null) {
        $('#pengguna').val(data[0].pengguna);
      }else {
        $('#pengguna').val(data[0].pengguna2);
      }
      $('#pengguna_awal').text(data[0].pengguna);
      ...
      if (data[0].no_sertifikat2=='' || data[0].no_sertifikat2==null) {
        $('#sertifikat').val(data[0].no_sertifikat);
      }else {
        $('#sertifikat').val(data[0].no_sertifikat2);
      }
      $('#sertifikat_awal').text(data[0].no_sertifikat);
      ...
      if (data[0].no_sensus=='' || data[0].no_sensus==null) {
        ...
        $('#no_header').text("A-"+data[0].no_sensus);
        $('#no_header').show();
      }
...
function batal(){
  ...
  $('#no_surat_pol').val('');
  $("#no_surat_pol").hide();
  $('#lat_awal').text('Lat. awal kosong!');
  $('#lon_awal').text('Lon. awal kosong!');
  $('#lat').val('');
  $('#lon').val('');
}
function submit(stat_submit) {
 ...
 var nilai_oleh     = $('#nilai_oleh').val();
 var kondisi_awal   = $('#kondisi_awal').val();
 var ket            = $('#ket').val();
...
$.ajax({
  url:"Trkib_a/simpan_trkib_a",
  method : "post",
  data:{stat_submit,id_brg,dtl_brg,luas,alamat,pengguna,lat,lon,sertifikat,thn_oleh,nilai_oleh,kondisi_awal,ket,kronologis,stat_fisik,keberadaan_brg,kondisi_brg,stat_hukum,ket_stat_hukum,bukti_milik,status_milik,ket_brg,no_surat_pol},
  async:false,
  dataType:"JSON",
  beforeSend: function (data) {
    swal({
...
function hapus_image(no,field){
  ...
  function(){
    $.ajax({
      dataType : 'json',
      type : 'POST',
      data : ({tabel:'trkib_a',field,id}),
      url : 'trkib_a/hapus_image',
      success : function(data){
        get_image();
      }
    });
