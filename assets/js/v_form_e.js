...
function uploadImage($form){
  ...
  request.open('post', 'Trkib_e/upload');
...
$(document).ready(function(){
  ...
  document.getElementById("sm7").style.color = "white";
  document.getElementById("sm7").style.fontWeight = "bold";
...
function sensus(id_barang,key){
  ...
  $.ajax({
    ...
    type : 'POST',
    data : {id_barang},
    url : 'Trkib_e/get_Trkib_e_id_non',
    ...
    success : function(data){
      ...
      if (data[0].judul2=='' || data[0].judul2==null) {
        $('#judul').val(data[0].judul);
      }else {
        $('#judul').val(data[0].judul2);
      } $('#judul_awal').text(data[0].judul);

      if (data[0].spesifikasi2=='' || data[0].spesifikasi2==null) {
        $('#spesifikasi').val(data[0].spesifikasi);
      }else {
        $('#spesifikasi').val(data[0].spesifikasi2);
      } $('#spesifikasi_awal').text(data[0].spesifikasi);

      if (data[0].asal2=='' || data[0].asal2==null) {
        $('#asal').val(data[0].asal);
      }else {
        $('#asal').val(data[0].asal2);
      } $('#asal_awal').text(data[0].asal);

      if (data[0].pencipta2=='' || data[0].pencipta2==null) {
        $('#pencipta').val(data[0].pencipta);
      }else {
        $('#pencipta').val(data[0].pencipta2);
      } $('#pencipta_awal').text(data[0].pencipta);
      ...
      if (data[0].jenis2=='' || data[0].jenis2==null) {
        ...
        $('#jenis').val(data[0].jenis2);
      } $('#jenis_awal').text(data[0].jenis);

      if (data[0].tipe2=='' || data[0].tipe2==null) {
        $('#tipe').val(data[0].tipe);
      }else {
        $('#tipe').val(data[0].tipe2);
      } $('#tipe_awal').text(data[0].tipe);
      ...
      if (data[0].no_sensus=='' || data[0].no_sensus==null) {
        ...
        $('#no_header').text("E-"+data[0].no_sensus);
        $('#no_header').show();
      }
...
$.ajax({
  url:"Trkib_e/simpan_Trkib_e",
  method : "post",
  data:{stat_submit,id_brg,dtl_brg,judul,spesifikasi,asal,pencipta,kd_bahan,jenis,tipe,ket,kronologis,stat_fisik,keberadaan_brg,kondisi_brg,stat_hukum,bukti_milik,status_milik,ket_brg,ket_stat_hukum,no_surat_pol,kd_ruang},
  async:false,
  dataType:"JSON",
  beforeSend: function (data) {
    swal({
...
function hapus_image(no,field){
  ...
  function(){
    $.ajax({
      dataType : 'json',
      type : 'POST',
      data : ({tabel:'trkib_e',field,id}),
      url : 'trkib_e/hapus_image',
      success : function(data){
        get_image();
      }
    });
