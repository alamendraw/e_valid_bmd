...
<script type="text/javascript">
	...
	function tambah() {
		...
		window.location.href = '<?php echo site_url('transaksi/C_Sewa/add'); ?>';
	}
	...
	function cari(){
		var key = $('#keyword').val();
		    $(function(){
			 $('#dg').datagrid({
				url: '<?php echo base_url();?>transaksi/C_Sewa/load_header',
				queryParams:({key:key})
				});        
			 });
	}
	...
	function hapus(){
		...
		if (row){
			$.messager.confirm('Konfirmasi','Yakin ingin menghapus data ini?',function(r){
				if (r){
					$.post('<?php echo base_url(); ?>transaksi/C_Sewa/hapus',
