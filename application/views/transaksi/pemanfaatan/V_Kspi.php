...
<script type="text/javascript">
	...
	function tambah() {
		...
		window.location.href = '<?php echo site_url('transaksi/C_Kspi/add'); ?>';
	}
	...
	$(document).ready(function() {
		...
		$('#dg').datagrid({
			...
			fitColumns:true,
			pagination:true,
			url: '<?php echo base_url(); ?>transaksi/C_Kspi/load_header',
	...
	function hapus(){
		...
		if (row){
			$.messager.confirm('Konfirmasi','Yakin ingin menghapus data ini?',function(r){
				if (r){
					$.post('<?php echo base_url(); ?>transaksi/C_Kspi/hapus',
