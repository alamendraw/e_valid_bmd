    ...
    <div class="row">
        <div class="col-md-6">
		 <div style="margin-bottom:10px">
			...
			<div style="margin-bottom:10px">
                <div class="col-sm-12" style="padding-bottom: 10px;">
                    <div class="col-sm-4" style="padding-top: 3px;"><label>Pemberi</label></div>
                    <div class="col-sm-8">
                        <input id="pemberi" name="pemberi" type="text" class="easyui-textbox" style="width:80%;">
                    </div>
                </div>
            </div>
        </div>
...
<div style="padding-bottom: 20px;">
    <button type="button" class="btn btn-default btn-lg btn-block" onClick="javascript:newUser();">Tambah Daftar Hibah Masuk</button>
</div>
...
<script type="text/javascript">
    ...
    window.onload = function(){
        ...
        if (status == 'detail') {
            ...
            var pemberi   		= localStorage.getItem('pemberi');
			...
			$("#pemberi").textbox('setValue',pemberi);
    ...
    function back() {
        localStorage.clear();
        window.location.href = "<?php echo base_url(); ?>transaksi/C_HibahMasuk";
    }

    function max_number(){ 
        $.ajax({
            type: "POST",
            url: '<?php echo base_url(); ?>transaksi/C_HibahMasuk/max_number',
	...
	function saveData(){
		...
		if(pemberi==''){
			iziToast.error({
					title: 'Error',
					message: 'Pemberi Tidak Boleh Kosong',
				});
				return
		}
		if(total==0){
		...
		$.post('<?php echo base_url(); ?>transaksi/C_HibahMasuk/saveData', 
		{detail: JSON.stringify(data1),no_dok:no_dokumen,tgl_dok:tgl_dokumen,kd_skpd:kd_skpd,kd_unit:kd_unit,pemberi:pemberi,status:status,tahun:tahun,no_urut:no_urut}, 
		function(result) {
		if (result.pesan){
				iziToast.success({
					title: 'OK',
    ...
    function newUser() {
        if (no_dokumen.value == "" || 
            ...
            tgl_dokumen.value == "") {
                ...
                iziToast.error({
                    title: 'Error',
                    message: 'Field No. Dokumen, SKPD, Pemberi, Tanggal Dokumen Harus Terisi.!',
                });
        } else {
    ...
    function load_detail() {
        ...
        $.ajax({
            url: '<?php echo site_url('transaksi/C_HibahMasuk/loadDetail') ?>',
            type: 'POST',
            dataType: 'json',
    ...
    $(document).ready(function() {
        ...
        $('#trd').datagrid({
			...
			nowrap:true,
			remoteSort:true,
            url: '<?php echo base_url(); ?>transaksi/C_HibahMasuk/loadDetail',
        ...
        $('#kd_skpd').combogrid({
            ...
            textField:'kd_skpd',  
            mode:'remote',
            url: '<?php echo base_url(); ?>transaksi/C_HibahMasuk/getSkpd',
            columns:[[  
            ...
            onSelect:function(rowIndex,rowData){
			   ...
			   $('#kd_unit').combogrid({url:'<?php echo base_url(); ?>transaksi/C_HibahMasuk/getUnit',
	...
	$('#kib').combobox({
        ...
        data: [{
			...
			unit = $('#kd_unit').combogrid('getValue');
             $('#no_reg').combogrid({url:'<?php echo base_url(); ?>transaksi/C_HibahMasuk/getDokumen',
        ...
        $('#no_reg').combogrid({
            ...
            textField:'no_reg',  
            mode:'remote',
            //url:'<?php echo base_url(); ?>transaksi/C_HibahMasuk/getSatuan',  
            columns:[[  
