...
<script type="text/javascript">
	...
	function newUser() {
		...
		window.location.href = '<?php echo site_url('transaksi/C_HibahMasuk/add'); ?>';
	}
	...
	function detail() {		
		...
		localStorage.setItem('pemberi', pemberi);
	...
	function hapus(){
		...
		var idu = [];
		var idv = [];
		for(var i=0; i<row.length; i++){ids.push(row[i].no_dokumen);idt.push(row[i].kd_skpd);idu.push(row[i].kd_unit);idv.push(row[i].no_urut);}
		var no_dokumen = ids.join('#');
		var kd_skpd = idt.join('#');
		var kd_unit = idu.join('#');
		var no_urut = idv.join('#');
		if (row){
			$.messager.confirm('Konfirmasi','Yakin ingin menghapus data ini?',function(r){
				if (r){
					$.post('<?php echo base_url(); ?>transaksi/C_HibahMasuk/hapus',
						{kode:no_dokumen,kd_skpd:kd_skpd,kd_unit:kd_unit,no_urut:no_urut},function(result){
						if (result.pesan){
							iziToast.info({
								title: 'OK',
	...
	$(document).ready(function() {
		...
		$('#dg').datagrid({
			...
			fitColumns:true,
			pagination:true,
			url: '<?php echo base_url(); ?>transaksi/C_HibahMasuk/load_header',
