...
class C_PinjamPakai extends CI_Controller {
	
	public function __construct()
	{
		parent::__construct();
		$this->load->model('transaksi/M_PinjamPakai');
	}
	public function index()
	{
		$data = array(
			'page' 		=> "Pinjam Pakai Barang",
			'judul'		=> "Data Pinjam Pakai Barang",
			'deskripsi'	=> "Pinjam Pakai Barang"
		);

		$this->template->views('transaksi/pemanfaatan/V_PinjamPakai', $data);
	}
	
		public function add()
	{
		$data = array(
			'page' 		=> "Tambah Pinjam Pakai Barang",
			'judul'		=> "Tambah Data Pinjam Pakai Barang",
			'deskripsi'	=> "Tambah Pinjam Pakai Barang"
		);
		$this->template->views('transaksi/pemanfaatan/V_Add_PinjamPakai', $data);
	}
	
	public function getKelompok()
	{
		$res = $this->M_PinjamPakai->getKelompok();
	...
	public function getJenis()
	{
		...
		$res = $this->M_PinjamPakai->getJenis($data);
		echo json_encode($res);
	}
	...
	public function getBarang()
	{
		...
		$result = $this->M_PinjamPakai->getBarang($data);
	...
	{
		$result = $this->M_PinjamPakai->getSkpd();
		echo json_encode($result);
	}
	public function getProgram()
	{
		$param = $this->input->post();
		$result = $this->M_PinjamPakai->getProgram($param);
		echo json_encode($result);
	}
	public function getKegiatan()
	{
		$param = $this->input->post();
		$result = $this->M_PinjamPakai->getKegiatan($param);
		echo json_encode($result);
	}
	...
	{
		$lccq 	= $this->input->post('q');
		$res = $this->M_PinjamPakai->getSatuan($lccq);
		echo json_encode($res);
	}
	...
	public function saveData(){
		...
		$h =	$this->M_PinjamPakai->simpan_header($header,$status,$no_dokumen,$kd_skpd);
		if($h == 1 ){
				$sukses =	$this->M_PinjamPakai->simpan_detail($data,$no_dokumen,$kd_skpd,$status);
					if($sukses){
	...
	public function hapus(){
		$param  = $this->input->post();
		$sukses = $this->M_PinjamPakai->hapus($param);
			if($sukses){
				echo json_encode(array('pesan'=>true));
			}else{
				echo json_encode(array('pesan'=>false));
	...
	function max_number(){
    	...
    	$result = $this->M_PinjamPakai->max_number($data);
        echo json_encode($result);
	}

	function trd_pinjampakai(){
    	...
    	$res = $this->M_PinjamPakai->load_trd($data);
    	echo json_encode($res);
    }
    ...
    function load_header()
	{
		...
		$sql = "SELECT count(a.*) as tot from transaksi.trh_pinjampakai a 
		where a.no_dokumen is not null $and1 $and2" ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
		
        $sql = "SELECT a.*,b.nm_skpd from transaksi.trh_pinjampakai a 
...
/* End of file C_PinjamPakai.php */
/* Location: ./application/controllers/transaksi/C_PinjamPakai.php */