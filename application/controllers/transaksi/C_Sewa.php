...
class C_Sewa extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('transaksi/M_Sewa');
	}
	public function index()
	{
		$data = array(
			'page' 		=> "Sewa Barang",
			'judul'		=> "Data Sewa Barang",
			'deskripsi'	=> "Sewa Barang"
		);

		$this->template->views('transaksi/pemanfaatan/V_Sewa', $data);
	}
	
		public function add()
	{
		$data = array(
			'page' 		=> "Tambah Sewa Barang",
			'judul'		=> "Tambah Data Sewa Barang",
			'deskripsi'	=> "Tambah Sewa Barang"
		);
		$this->template->views('transaksi/pemanfaatan/V_Add_Sewa', $data);
	}
	
	public function getKelompok()
	{
		$res = $this->M_Sewa->getKelompok();
	...
	public function getBarang()
	{
		...
		$result = $this->M_Sewa->getBarang($data);
	...
	{
		$result = $this->M_Sewa->getSkpd();
		echo json_encode($result);
	}
	public function getProgram()
	{
		$param = $this->input->post();
		$result = $this->M_Sewa->getProgram($param);
		echo json_encode($result);
	}
	public function getKegiatan()
	{
		$param = $this->input->post();
		$result = $this->M_Sewa->getKegiatan($param);
		echo json_encode($result);
	}
	...
	{
		$lccq 	= $this->input->post('q');
		$res = $this->M_Sewa->getSatuan($lccq);
		echo json_encode($res);
	}
	
	public function saveData(){
		$no_dokumen	= $this->input->post('no_dok');
		$tgl_dokumen= $this->M_Sewa->tanggal_balik($this->input->post('tgl_dok'));
		...
		$h =	$this->M_Sewa->simpan_header($header,$status,$no_dokumen,$kd_skpd);
		if($h == 1 ){
				$sukses =	$this->M_Sewa->simpan_detail($data,$no_dokumen,$kd_skpd,$status);
					if($sukses){
	...
	public function hapus(){
		$param  = $this->input->post();
		$sukses = $this->M_Sewa->hapus($param);
			if($sukses){
				echo json_encode(array('pesan'=>true));
			}else{
				echo json_encode(array('pesan'=>false));
	...
	function max_number(){
    	...
    	$result = $this->M_Sewa->max_number($data);
        echo json_encode($result);
	}
    ...
    function load_header()
	{
		...
		$sql = "SELECT count(a.*) as tot from transaksi.trh_sewa a 
		where a.no_dokumen is not null $and1 $and2" ;
        $query1 = $this->db->query($sql);
        $total = $query1->row();
		
        $sql = "SELECT a.*,b.nm_skpd from transaksi.trh_sewa a 
        ...
        { 
            $row[] = array(
                ...
                'tgl_dokumen' 	=> $this->M_Sewa->tanggal_balik($resulte['tgl_dokumen']),
                'tahun' 		=> $resulte['tahun'],
...
/* End of file C_Sewa.php */
/* Location: ./application/controllers/transaksi/C_Sewa.php */