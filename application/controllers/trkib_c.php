...
class Trkib_c extends CI_Controller {
	...
	public function index(){
		...
		if($session == FALSE)
		{
			...
			$a['page']  ='v_form_c';
			$a['title'] ='GEDUNG DAN BANGUNAN';
			$a['icon']  ='fa fa-building';
			$this->load->view('main',$a);
		}
	}
	function get_trkib_c_non(){
			...
			if(!empty($_FILES['file_'.$i]['name'])){
				...
				if(!$this->upload->do_upload('file_'.$i))
					...
					if($i=='1'){
						...
						$query = $this->db->query("SELECT foto1 FROM trkib_c where id_barang ='$p'")->row('foto1');
						for ($i=''; $i <= 4; $i++) { 
						...
						$this->db->query("UPDATE trkib_c set foto1 = '$foto1[file_name]' WHERE id_barang = '$p'");
					}else if($i=='2'){
						$foto2 = $this->upload->data();
						$query = $this->db->query("SELECT foto2 FROM trkib_c where id_barang ='$p'")->row('foto2');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_c set foto2 = '$foto2[file_name]' WHERE id_barang = '$p'");
					}else if($i=='3'){
						$foto3 = $this->upload->data();
						$query = $this->db->query("SELECT foto3 FROM trkib_c where id_barang ='$p'")->row('foto3');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_c set foto3 = '$foto3[file_name]' WHERE id_barang = '$p'");
					}else if($i=='4'){
						$foto4 = $this->upload->data();
						$query = $this->db->query("SELECT foto4 FROM trkib_c where id_barang ='$p'")->row('foto4');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_c set foto4 = '$foto4[file_name]' WHERE id_barang = '$p'");
					}
	...
	function get_trkib_c_id_non(){
		$p = $this->input->post('id_barang');
		$query = $this->db->query("SELECT * from trkib_c where  id_barang = '$p'")->result();
		echo json_encode($query);
	}
	function simpan_trkib_c(){
		$p = $this->input->post('id_brg');
		$cek_num = $this->db->query("SELECT no_sensus FROM trkib_c WHERE id_barang='$p'")->row_array();
		if ($cek_num['no_sensus']=='' OR $cek_num['no_sensus']==null) {
			$set_num = $this->db->query("SELECT LPAD(IFNULL(MAX(no_sensus),0)+1,6,'0') AS nor FROM trkib_c WHERE kd_unit='$_SESSION[unit_skpd]'")->row_array();
			$num = $set_num['nor'];
		}else {
		...
		if ($this->input->post('stat_fisik')==1) {
			if ($this->input->post('stat_submit')==1) {
				$data=array(
					...
					'konstruksi3'    => $this->input->post('konstruksi'),
					'jenis_gedung2'  => $this->input->post('jns_gedung'),
					'luas_lantai2'   => $this->input->post('luas'),
		...
		$query = $this->db->update('trkib_c',$data);
		$msg   = $this->db->error();
		if ($query) {
			$p = array(
				'title' => "Good Job!",
	...
	function batal_trkib_c(){
		...
		$this->db->where('id_barang', $p);
		$cek = $this->db->query("SELECT id_barang FROM trkib_c WHERE id_barang='$p' AND (
	...
	function get_image(){
		...
		$query = $this->db->query("
			...
			from trkib_c where id_barang = '$p'");
		$data = array();
