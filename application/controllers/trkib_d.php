...
class Trkib_d extends CI_Controller {
	...
	public function index(){
		...
		if($session == FALSE)
		{
			...
			$a['page']  ='v_form_d';
			$a['title'] ='JALAN, IRIGASI DAN JARINGAN';
			$a['icon']  ='fa fa-road';
			$this->load->view('main',$a);
		}
	}
	function get_trkib_d_non(){
		$kd_skpd = $this->input->post('kd_skpd');
		$kd_unit = $this->input->post('kd_unit');
		if ($_SESSION['otori']==1 OR $_SESSION['otori']==4) {
			if ($kd_unit=='' OR $kd_unit==null) {
				$query=$this->db->select(array('no_sensus','id_barang','kd_brg','nm_brg','asal','nilai','kd_skpd','kd_unit','tahun'))
				->from('trkib_d')
		...
		$column_order = array(null,'no_sensus','id_barang','kd_brg','nm_brg','asal','nilai','kd_skpd','kd_unit','tahun',null);
		$column_search = array('no_sensus','id_barang','kd_brg','nm_brg','asal','nilai','kd_skpd','kd_unit','tahun','keterangan');
			...
			if(!empty($_FILES['file_'.$i]['name'])){
				...
				if(!$this->upload->do_upload('file_'.$i))
					...
					if($i=='1'){
						...
						$query = $this->db->query("SELECT foto as foto1 FROM trkib_d where id_barang ='$p'")->row('foto1');
						for ($i=''; $i <= 4; $i++) { 
						...
						$this->db->query("UPDATE trkib_d set foto = '$foto1[file_name]' WHERE id_barang = '$p'");
					}else if($i=='2'){
						$foto2 = $this->upload->data();
						$query = $this->db->query("SELECT foto2 FROM trkib_d where id_barang ='$p'")->row('foto2');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_d set foto2 = '$foto2[file_name]' WHERE id_barang = '$p'");
					}else if($i=='3'){
						$foto3 = $this->upload->data();
						$query = $this->db->query("SELECT foto3 FROM trkib_d where id_barang ='$p'")->row('foto3');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_d set foto3 = '$foto3[file_name]' WHERE id_barang = '$p'");
					}else if($i=='4'){
						$foto4 = $this->upload->data();
						$query = $this->db->query("SELECT foto4 FROM trkib_d where id_barang ='$p'")->row('foto4');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_d set foto4 = '$foto4[file_name]' WHERE id_barang = '$p'");
					}
	...
	function get_trkib_d_id_non(){
		$p = $this->input->post('id_barang');
		$query = $this->db->query("SELECT * from trkib_d where id_barang = '$p'")->result();
		echo json_encode($query);
	}
	function simpan_trkib_d(){
		$p = $this->input->post('id_brg');
		$cek_num = $this->db->query("SELECT no_sensus FROM trkib_d WHERE id_barang='$p'")->row_array();
		if ($cek_num['no_sensus']=='' OR $cek_num['no_sensus']==null) {
			$set_num = $this->db->query("SELECT LPAD(IFNULL(MAX(no_sensus),0)+1,6,'0') AS nor FROM trkib_d WHERE kd_unit='$_SESSION[unit_skpd]'")->row_array();
			$num = $set_num['nor'];
		}else {
		...
		if ($this->input->post('stat_fisik')==1) {
			...
			if ($this->input->post('stat_submit')==1) {
				$data=array(
					...
					'konstruksi2'    => $this->input->post('konstruksi'),
					'panjang2'       => $this->input->post('panjang'),
					'lebar2'         => $this->input->post('lebar'),
					'luas2'          => $this->input->post('luas'),
					'tgl_dok2'       => date("Y-m-d", strtotime($this->input->post('tgl_dok'))),
					'no_dok2'        => $this->input->post('no_dok'),
		...
		$query = $this->db->update('trkib_d',$data);
		$msg   = $this->db->error();
		if ($query) {
			$p = array(
				'title' => "Good Job!",
	...
	function get_image(){
		...
		$query = $this->db->query("
			...
			from trkib_d where id_barang = '$p'");
		$data = array();
		$foto1 = '';
		$foto2 = '';
		$foto3 = '';
