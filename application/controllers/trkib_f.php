...
class Trkib_f extends CI_Controller {
	...
	public function index(){
		...
		if($session == FALSE)
		{
			...
			$a['page']  ='v_form_f';
			$a['title'] ='KONSTRUKSI DALAM PENGERJAAN';
			$a['icon']  ='fa fa-traffic-cone ';
			$this->load->view('main',$a);
		}
	}
	function get_trkib_f_non(){
	...
	function get_trkib_f_id_non(){
		$p = $this->input->post('id_barang');
		$query = $this->db->query("SELECT * from trkib_f where id_barang = '$p'")->result();
		echo json_encode($query);
	}
	function simpan_trkib_f(){
		$p = $this->input->post('id_brg');
		$cek_num = $this->db->query("SELECT no_sensus FROM trkib_f WHERE id_barang='$p'")->row_array();
		if ($cek_num['no_sensus']=='' OR $cek_num['no_sensus']==null) {
			$set_num = $this->db->query("SELECT LPAD(IFNULL(MAX(no_sensus),0)+1,6,'0') AS nor FROM trkib_f WHERE kd_unit='$_SESSION[unit_skpd]'")->row_array();
			$num = $set_num['nor'];
		}else {
		...
		if ($this->input->post('stat_fisik')==1) {
			...
			if ($this->input->post('stat_submit')==1) {
				$data=array(
					...
					'bangunan2'       => $this->input->post('bangunan'),
					'konstruksi2'     => $this->input->post('konstruksi'),
					'jenis2'          => $this->input->post('jns_gedung'),
					'luas2'           => $this->input->post('luas'),
					'alamat'          => $this->input->post('alamat'),
					'lat2'            => $this->input->post('lat'),
					'lon2'            => $this->input->post('lon'),
					'tgl_awal_kerja2' => date("Y-m-d",strtotime($this->input->post('tgl_mulai'))),
		...
		$query = $this->db->update('trkib_f',$data);
		$msg   = $this->db->error();
		if ($query) {
			$p = array(
				'title' => "Good Job!",
	...
	function get_image(){
		...
		$query = $this->db->query("
			...
			from trkib_f where id_barang = '$p'");
		$data = array();
		$foto1 = '';
		$foto2 = '';
		$foto3 = '';
