...
class Trkib_b extends CI_Controller {
	...
	function get_trkib_b_non(){
		$kd_skpd = $this->input->post('kd_skpd');
		$kd_unit = $this->input->post('kd_unit');
		if ($_SESSION['otori']==1 OR $_SESSION['otori']==4) {
			if ($kd_unit=='' OR $kd_unit==null) {
				$query=$this->db->select(array('no_sensus','id_barang','kd_brg','nm_brg','asal','nilai','kd_skpd','kd_unit','tahun','detail_brg','merek'))
				->from('trkib_b')
		...
		$column_order = array(null,'no_sensus','id_barang','kd_brg','nm_brg','asal','nilai','kd_skpd','kd_unit','tahun','detail_brg','merek',null);
		$column_search = array('no_sensus','id_barang','kd_brg','nm_brg','asal','nilai','kd_skpd','kd_unit','tahun','detail_brg','merek','keterangan','no_rangka','no_mesin','no_polisi');
	...
	function upload(){
		...
		//$config['upload_path'] = './assets/image_file'; 
		...
		for ($i=1; $i <=4 ; $i++) { 
			if(!empty($_FILES['file_'.$i]['name'])){
				$config['file_name'] = md5($_FILES['file_'.$i]['name'].time().date('Y-m-d H:i:s'));
				$this->load->library('upload', $config);
				$this->upload->initialize($config);
				if(!$this->upload->do_upload('file_'.$i))
					...
					if($i=='1'){
						...
						$query = $this->db->query("SELECT foto as foto1 FROM trkib_b where id_barang ='$p'")->row('foto1');
						for ($i=''; $i <= 4; $i++) { 
						...
						$this->db->query("UPDATE trkib_b set foto = '$foto1[file_name]' WHERE id_barang = '$p'");
					}else if($i=='2'){
						$foto2 = $this->upload->data();
						$query = $this->db->query("SELECT foto2 FROM trkib_b where id_barang ='$p'")->row('foto2');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_b set foto2 = '$foto2[file_name]' WHERE id_barang = '$p'");
					}else if($i=='3'){
						$foto3 = $this->upload->data();
						$query = $this->db->query("SELECT foto3 FROM trkib_b where id_barang ='$p'")->row('foto3');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_b set foto3 = '$foto3[file_name]' WHERE id_barang = '$p'");
					}else if($i=='4'){
						$foto4 = $this->upload->data();
						$query = $this->db->query("SELECT foto4 FROM trkib_b where id_barang ='$p'")->row('foto4');
						for ($i=''; $i <= 4; $i++) { 
							if (file_exists(FCPATH.'assets/image_file'.$i.'/'.$query)) {
								$path_file = './assets/image_file'.$i.'/';
								unlink($path_file.$query);
						...
						$this->db->query("UPDATE trkib_b set foto4 = '$foto4[file_name]' WHERE id_barang = '$p'");
					}
	...
	function get_trkib_b_id_non(){
		$p = $this->input->post('id_barang');
		$query = $this->db->query("SELECT a.*,concat(b.kd_ruang,' | ',b.nm_ruang,' | ',b.no_urut) as ruang_lama from trkib_b a left join mruang b on a.kd_ruang=b.kd_ruang where id_barang = '$p'")->result();
		echo json_encode($query);
	}
	function simpan_trkib_b(){
		$p = $this->input->post('id_brg');
		$cek_num = $this->db->query("SELECT no_sensus FROM trkib_b WHERE id_barang='$p'")->row_array();
		if ($cek_num['no_sensus']=='' OR $cek_num['no_sensus']==null) {
			$set_num = $this->db->query("SELECT LPAD(IFNULL(MAX(no_sensus),0)+1,6,'0') AS nor FROM trkib_b WHERE kd_unit='$_SESSION[unit_skpd]'")->row_array();
			$num = $set_num['nor'];
		}else {
		...
		if ($this->input->post('stat_fisik')==1) {
			...
			if ($this->input->post('stat_submit')==1) {
				$data=array(
					...
					'merek2'         => $this->input->post('merek'),
					'silinder2'      => $this->input->post('ukuran'),
					'kd_bahan2'      => $this->input->post('kd_bahan'),
					'pabrik2'        => $this->input->post('pabrik'),
					'no_rangka2'     => $this->input->post('no_rangka'),
					'no_mesin2'      => $this->input->post('no_mesin'),
					'no_polisi2'     => $this->input->post('no_pol'),
					'keterangan2'    => $this->input->post('ket'),
					'no_bpkb2'       => $this->input->post('no_bpkb'),
		...
		$cek = $this->db->query("SELECT id_barang FROM trkib_b WHERE id_barang='$p' AND (
			LENGTH(foto)=36 OR LENGTH(foto)=37 OR
		...
		if ($this->input->post('stat_submit')==0) {
			$query = $this->db->update('trkib_b',$data);
			if ($query) {
				$success = true;
			}
		}elseif (count($cek)>0 && $this->input->post('stat_fisik')==1) {
			$query = $this->db->update('trkib_b',$data);
			if ($query) {
				$success = true;
			}
	...
	function get_image(){
		...
		$query = $this->db->query("
			...
			from trkib_b where id_barang = '$p'");
		$data = array();
		$foto1 = '';
		$foto2 = '';
		$foto3 = '';
	...
	{
		$no = 1;
		$show = $this->db->query("SHOW FULL PROCESSLIST")->result_array();
		foreach ($show as $row) {
			echo($row["Id"]);
			if ($row["Time"] > 200 ) {
				$sql="KILL $row[Id]";
