...
class M_HibahMasuk extends CI_Model {
	...
	function saveHeader($post){
		...
		$query = "INSERT INTO transaksi.trh_hibah(no_urut,tahun,no_dokumen,tgl_dokumen,pemberi,tujuan,kd_skpd,kd_unit,jenis,last_update,username)
					VALUES('$no_urut','$tahun','$no_dokumen','$tgl_dokumen','$pemberi','','$kd_skpd','$kd_unit','1','$tgl_update','$username')";
	...
	function updateHeader($post){
		...
		$query = "UPDATE transaksi.trh_hibah SET 
			...
			username    = '$username',
			tahun		= '$tahun'
			WHERE no_urut = '$no_urut' AND kd_skpd ='$kd_skpd' AND kd_unit = '$kd_unit' AND jenis = '1'";
			 $sql = $this->db->query($query);  
			try{
			if ($sql) {
				return 1;
	...
	function saveDetail($post,$no_dokumen,$kd_skpd,$kd_unit,$no_urut){
		foreach ($post as $row){
		...
		$query = "INSERT INTO transaksi.trd_hibah(no_dokumen,id_reg,
					kd_brg,nilai,kib,kd_skpd,kd_unit,jenis,no_urut)
					VALUES('$no_dokumen','$no_reg','$kd_brg','$nilai',
					'$jns_kib','$kd_skpd','$kd_unit','1','$no_urut')";
		$sql = $this->db->query($query);  
		}
		try{
			if ($sql) {
	...
	function updateDetail($post,$no_dokumen,$kd_skpd,$kd_unit,$no_urut){
		$del 	= "DELETE FROM transaksi.trd_hibah WHERE no_urut='$no_urut' AND kd_skpd='$kd_skpd' AND kd_unit='$kd_unit' AND jenis='1'";
		...
		$query = "INSERT INTO transaksi.trd_hibah(no_dokumen,id_reg,kd_brg,nilai,kib,kd_skpd,kd_unit,jenis,no_urut)
					VALUES('$no_dokumen','$no_reg','$kd_brg','$nilai','$jns_kib','$kd_skpd','$kd_unit','1','$no_urut')";
		$sql = $this->db->query($query);  
			}
		try{
			if ($sql) {
	...
	public function loadDetail($param){
		...
		where no_dokumen = '$nomor' and kd_skpd = '$skpd' AND kd_unit = '$unit' AND jenis='1' AND no_urut='$urut'";
        $rs   = $this->db->query($csql)->row() ; 
        $sql = "SELECT a.*, b.uraian as nm_barang FROM transaksi.trd_hibah a LEFT JOIN public.mbarang b 
				ON a.kd_brg=b.kd_brg WHERE a.no_dokumen = '$nomor' AND a.kd_skpd = '$skpd' AND a.kd_unit='$unit' AND a.jenis='1' AND a.no_urut='$urut' ";
	...
	public function getUnit($lccq,$skpd){
		$sql	= "SELECT kd_unit,nm_unit FROM public.munit WHERE kd_skpd='$skpd' AND (UPPER(kd_unit) LIKE UPPER('%$lccq%') OR UPPER(nm_unit) LIKE UPPER('%$lccq%'))order by kd_unit";
	...
	public function getDokumen($data){
		...
		$sql	= "select no_reg,a.kd_brg,nilai,keterangan,b.uraian as nm_brg FROM transaksi.$from a 
				LEFT JOIN public.mbarang b ON a.kd_brg=b.kd_brg WHERE a.tgl_reg <='$tgl'
				AND a.kd_skpd = '$skpd' $uskpd 
				/*AND CONCAT(a.no_reg,a.kd_brg,a.kd_skpd,a.kd_unit) NOT IN (SELECT CONCAT(no_reg,kd_brg,kd_skpd,kd_unit) FROM transaksi.trd_hibah WHERE jenis='1')*/";
				//AND a.asal = '01'
		$query  = $this->db->query($sql);
		$res = array();
		$li = 0;
	...
	public function max_number($tahun,$skpd){
		$query1 = $this->db->query("SELECT COALESCE(MAX(CAST(no_urut as int)),0) as nomor FROM transaksi.trh_hibah WHERE kd_skpd='$skpd' AND tahun='$tahun' AND jenis='1'");  
