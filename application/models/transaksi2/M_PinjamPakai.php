...
class M_PinjamPakai extends CI_Model {
	
	function simpan_header($post,$status,$no_dok,$skpd){

		try {
			if($status!='detail'){			
				$ck = $this->db->query("SELECT no_dokumen FROM transaksi.trh_pinjampakai
                           WHERE no_dokumen = '$no_dok' and kd_uskpd='$skpd'");	
				if($ck->num_rows() == 0) {
					$this->db->insert('transaksi.trh_pinjampakai', $post);
						return 1;
				} else {
						return 0;
				}
				...
				$del = $this->db->where('no_dokumen',$post['no_dokumen'])
							...
							->delete('transaksi.trh_pinjampakai');
				if($del){
				$sql = $this->db->insert('transaksi.trh_pinjampakai', $post);
				}
				if ($sql) {
					return 1;
					$sql->free_result();
			...
			if($status!='detail'){						
					foreach($post as $row) {							
							$filter_data = array(
								...
								"total" => str_replace(array(','), array(''), $row->biaya_pinjampakai),
								"status" => '',
								"kd_rek" => '',
								"biaya_pinjampakai" => str_replace(array(','), array(''), $row->biaya_pinjampakai),
								"uraian_pinjampakai" => str_replace(array(','), array(''), $row->uraian_pinjampakai),
								"ket" => htmlspecialchars($row->uraian_pinjampakai, ENT_QUOTES),
								"satuan" => htmlspecialchars($row->satuan, ENT_QUOTES),
								"kd_kegiatan" => htmlspecialchars($row->kd_kegiatan, ENT_QUOTES)
							);
						$sql = $this->db->insert('transaksi.trd_pinjampakai', $filter_data);
						}
			}else{
				$del = $this->db->where('no_dokumen',$no_dokumen)
							->where('kd_uskpd',$kd_skpd)
							->delete('transaksi.trd_pinjampakai');
				if($del){
						foreach($post as $row) {							
							$filter_data = array(
								"no_dokumen" => htmlspecialchars($no_dokumen, ENT_QUOTES),
	...
	public function load_trd($param)
	{
		...
		$csql = "SELECT SUM(total) AS total from transaksi.trd_pinjampakai 
		where no_dokumen = '$nomor' and kd_uskpd = '$skpd'";
        $rs   = $this->db->query($csql)->row() ; 
		
        $sql = "SELECT b.* FROM transaksi.trh_pinjampakai a 
				INNER JOIN transaksi.trd_pinjampakai b ON a.no_dokumen=b.no_dokumen 
        ...
        {            
            $result[] = array(                                
                ...
                'total'         => $resulte['total'],
                'sum'         	=> $rs->total,                        
                'biaya_pinjampakai'=> $resulte['biaya_pinjampakai'],           
                'uraian_pinjampakai'=> $resulte['uraian_pinjampakai'],          
                'ket'           => $resulte['ket'],                        
