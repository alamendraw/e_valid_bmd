...
class M_Kspi extends CI_Model {
	
	function simpan_header($post,$status,$no_dok,$skpd){

		try {
			if($status!='detail'){				
				$ck = $this->db->query("SELECT no_dokumen FROM transaksi.trh_kspi
                           WHERE no_dokumen = '$no_dok' and kd_uskpd='$skpd'");	
				if($ck->num_rows() == 0) {
					$this->db->insert('transaksi.trh_kspi', $post);
						return 1;
				} else {
						return 0;
				}
				...
				$del = $this->db->where('no_dokumen',$post['no_dokumen'])
							...
							->delete('transaksi.trh_kspi');
				if($del){
				$sql = $this->db->insert('transaksi.trh_kspi', $post);
				}
				if ($sql) {
					return 1;
					$sql->free_result();
			...
			if($status!='detail'){						
					foreach($post as $row) {							
							$filter_data = array(
								...
								"total" => str_replace(array(','), array(''), $row->biaya_kspi),
								"status" => '',
								"kd_rek" => '',
								"biaya_kspi" => str_replace(array(','), array(''), $row->biaya_kspi),
								"uraian_kspi" => str_replace(array(','), array(''), $row->uraian_kspi),
								"ket" => htmlspecialchars($row->uraian_kspi, ENT_QUOTES),
								"satuan" => htmlspecialchars($row->satuan, ENT_QUOTES),
								"kd_kegiatan" => htmlspecialchars($row->kd_kegiatan, ENT_QUOTES)
							);
						$sql = $this->db->insert('transaksi.trd_kspi', $filter_data);
						}
			}else{
				$del = $this->db->where('no_dokumen',$no_dokumen)
							->where('kd_uskpd',$kd_skpd)
							->delete('transaksi.trd_kspi');
				if($del){
						foreach($post as $row) {							
							$filter_data = array(
								"no_dokumen" => htmlspecialchars($no_dokumen, ENT_QUOTES),
	...
	public function load_trd($param)
	{
		...
		$csql = "SELECT SUM(total) AS total from transaksi.trd_kspi 
		where no_dokumen = '$nomor' and kd_uskpd = '$skpd'";
        $rs   = $this->db->query($csql)->row() ; 
		
        $sql = "SELECT b.* FROM transaksi.trh_kspi a 
				INNER JOIN transaksi.trd_kspi b ON a.no_dokumen=b.no_dokumen 
        ...
        {            
            $result[] = array(                                
                ...
                'total'         => $resulte['total'],
                'sum'         	=> $rs->total,                        
                'biaya_kspi'=> $resulte['biaya_kspi'],           
                'uraian_kspi'=> $resulte['uraian_kspi'],          
                'ket'           => $resulte['ket'],                        
	...
	public function getJenis($param)
	{
		...
		if ($kel != '') {
			if($kel=='13'){
			$whr = "LEFT(kd_brg, 2) = '$kel' AND LENGTH(kd_brg)='3' AND kd_brg in('133','134') AND ";
			}else{
