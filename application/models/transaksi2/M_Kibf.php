...
class M_Kibf extends CI_Model {
	...
	function saveData($post){
			...
			for ($x = 1; $x <= $jumlah; $x++) {
				...
				$query ="insert into transaksi.trkib_f (
					no_reg,id_barang,id_lokasi, kd_brg, no_dokumen, milik, wilayah, kd_skpd, kd_unit, asal, dsr_peroleh, no_oleh, tgl_oleh, tahun, nilai, jumlah, tgl_reg, kondisi, konstruksi, konstruksi2, bangunan, luas,tgl_awal_kerja,status_tanah,nilai_kontrak,alamat1, alamat2, alamat3, keterangan, lat, lon, kd_tanah, foto1, foto2, foto3, foto4,total, username,tgl_update,auto,sts,nm_brg,detail_brg,ket_matriks
				)
				values ( '$no_regis', '$id_barang', '$id_barang', '$kd_barang', '$no_dokumen', '$milik', '$wil', '$kd_skpd', '$kd_unit', '$perolehan', '$dasar', '$no_oleh', '$tgl_oleh', '$thn_oleh', '$hrg_oleh', '1', '$tgl_regis', '$kondisi', '$konstruksi', '$konstruksi2', '$jenis', '$luas', '$tgl_mulai', '$sts_tanah', '$kontrak','$alamat1', '$alamat2', '$alamat3', '$keterangan', cast(NULLIF('$latitude','') as double precision),cast(NULLIF('$longtitude','') as double precision), '$kd_tanah', '$gambar1', '$gambar2', '$gambar3', '$gambar4', '$hrg_oleh','$user', CURRENT_TIMESTAMP, '$auto','0','$nm_brg','$detail','$ket_matriks'
				);";
				// print_r($query);
	...
	function getAuto(){
		$query = $this->db->query("SELECT max(auto) as auto from transaksi.trkib_f")->row('auto');
		if($query=='' || $query=='NULL'){
			$hasil = '1';
		}else{
			$hasil = $query +1;
	...
	function editData($post){
		...
		switch ($sts) {
			case 1:
				$query = "UPDATE transaksi.trkib_f SET 
						milik			= '$milik',
						wilayah			= '$wil',
						asal			= '$perolehan',
						dsr_peroleh		= '$dasar',
		...
		try{
			if(count($ex) > 0){
				foreach($ex as $idx=>$val){
					$query = "SELECT foto1,foto2,foto3,foto4 from transaksi.trkib_f where id_lokasi='$val' and sts!='1'";
					$sql = $this->db->query($query)->row_array();
					for ($i=1; $i <= 4; $i++) {
						if ($sql['foto'.$i]!='' || $sql['foto'.$i]!=null) {
							if (file_exists('./uploads/kibF/'.$sql['foto'.$i])) {
								$path1 ='./uploads/kibF/'.$sql['foto'.$i];
								unlink($path1);
							}
						}
					}
					$query = "DELETE from transaksi.trkib_f where id_lokasi='$val' and sts!='1'";
					$sql = $this->db->query($query);
				}
	...
	public function loadHeader($key1,$key2,$key3) {
		...
		if($otori=='01'){
			...
			if($key3!=''){ //kondisi / keterangan / luas / sumber dana / alamat
				$xkey3 = "and (upper(g.cara_peroleh) like upper('%$key3%') or upper(a.alamat1) like upper('%$key3%') or upper(a.luas) like upper('%$key3%') or upper(a.kondisi) like upper('%$key3%') or upper(a.keterangan) like upper('%$key3%') or a.nilai::text like '%$key3%')";	 
			}
			if($key1!='' || $key2!='' || $key3!=''){
				$where = "where a.no_reg!='' $kondisi $xkey1 $xkey2 $xkey3";	
			}
			
			$sql = "SELECT count(*) as tot from transaksi.trkib_f a 
			...
			foreach($query1->result_array() as $resulte){
				...
				$row[] = array(
					...
					'jenis'         => $resulte['bangunan'],
					'luas'          => $resulte['luas'],
					'tgl_mulai'     => $this->tanggal_ind($resulte['tgl_awal_kerja']),
					'sts_tanah'     => $resulte['status_tanah'],
					'kontrak'       => $resulte['nilai_kontrak'],
	...
	public function loadBarang($akun,$kel,$jenis,$rincian,$key) {  
		...
		$sql = "SELECT count(*) as tot FROM public.mbarang 
				...
				and kelompok='$kel' and jenis='$jenis' and left(kd_brg,14)='$rincian'
				$where" ;
	...
	public function getJenisBangun($lccq){
		$sql	= "SELECT kode,jns_bangunan FROM public.mjenis group by kode,jns_bangunan order by kode ";
		$query  = $this->db->query($sql);
		$res = array();
		$li = 0;
		foreach ($query->result_array() as $key) {
	...
	public function getKonstruksi($lccq){
		$sql	= "SELECT kode,nm_konstruksi FROM public.mkonstruksi group by kode,nm_konstruksi order by kode ";
		$query  = $this->db->query($sql);
		$res = array();
		$li = 0;
		foreach ($query->result_array() as $key) {
	...
	public function getKonstruksi2($lccq){
		$sql	= "SELECT kode,nm_konstruksi FROM public.mkonstruksi2 group by kode,nm_konstruksi order by kode ";
		$query  = $this->db->query($sql);
		$res = array();
		$li = 0;
		foreach ($query->result_array() as $key) {
	...
	public function max_number($tahun,$skpd,$kd_brg){
		...
		FROM transaksi.trkib_f WHERE kd_skpd='$skpd' AND tahun='$tahun' and kd_brg='$kd_brg'");  
