...
class M_BgsBsg extends CI_Model {
	
	function simpan_header($post,$status,$no_dok,$skpd){

		try {
			if($status!='detail'){			
				$ck = $this->db->query("SELECT no_dokumen FROM transaksi.trh_bgsbsg
                           WHERE no_dokumen = '$no_dok' and kd_uskpd='$skpd'");	
				if($ck->num_rows() == 0) {
					$this->db->insert('transaksi.trh_bgsbsg', $post);
						return 1;
				} else {
						return 0;
				}
				...
				$del = $this->db->where('no_dokumen',$post['no_dokumen'])
							...
							->delete('transaksi.trh_bgsbsg');
				if($del){
				$sql = $this->db->insert('transaksi.trh_bgsbsg', $post);
				}
				if ($sql) {
					return 1;
					$sql->free_result();
			...
			if($status!='detail'){						
					foreach($post as $row) {							
							$filter_data = array(
								...
								"total" => str_replace(array(','), array(''), $row->biaya_bgsbsg),
								"status" => '',
								"kd_rek" => '',
								"biaya_bgsbsg" => str_replace(array(','), array(''), $row->biaya_bgsbsg),
								"uraian_bgsbsg" => str_replace(array(','), array(''), $row->uraian_bgsbsg),
								"ket" => htmlspecialchars($row->uraian_bgsbsg, ENT_QUOTES),
								"satuan" => htmlspecialchars($row->satuan, ENT_QUOTES),
								"kd_kegiatan" => htmlspecialchars($row->kd_kegiatan, ENT_QUOTES)
							);
						$sql = $this->db->insert('transaksi.trd_bgsbsg', $filter_data);
						}
			}else{
				$del = $this->db->where('no_dokumen',$no_dokumen)
							->where('kd_uskpd',$kd_skpd)
							->delete('transaksi.trd_bgsbsg');
				if($del){
						foreach($post as $row) {							
							$filter_data = array(
								"no_dokumen" => htmlspecialchars($no_dokumen, ENT_QUOTES),
	...
	public function load_trd($param)
	{
		...
		$csql = "SELECT SUM(total) AS total from transaksi.trd_bgsbsg 
		where no_dokumen = '$nomor' and kd_uskpd = '$skpd'";
        $rs   = $this->db->query($csql)->row() ; 
		
        $sql = "SELECT b.* FROM transaksi.trh_bgsbsg a 
				INNER JOIN transaksi.trd_bgsbsg b ON a.no_dokumen=b.no_dokumen 
        ...
        {            
            $result[] = array(                                
                ...
                'total'         => $resulte['total'],
                'sum'         	=> $rs->total,                        
                'biaya_bgsbsg'=> $resulte['biaya_bgsbsg'],           
                'uraian_bgsbsg'=> $resulte['uraian_bgsbsg'],          
                'ket'           => $resulte['ket'],                        
	...
	public function getJenis($param)
	{
		...
		if ($kel != '') {
			if($kel=='13'){
			$whr = "LEFT(kd_brg, 2) = '$kel' AND LENGTH(kd_brg)='3' AND kd_brg='131' AND ";
			}else{
	...
	public function getBarang($param)
	{
		...
		$sql ="SELECT a.kd_brg,b.uraian,'buah' as kd_satuan,'' as merek,
		...
		or upper(b.uraian) like upper('%$lccq%')) ORDER BY a.kd_brg LIMIT 100 OFFSET 1";		
		$query = $this->db->query($sql);
		$res = array();
		$li = 0;
		foreach ($query->result_array() as $key) {
