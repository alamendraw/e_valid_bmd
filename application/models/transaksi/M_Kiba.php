...
class M_Kiba extends CI_Model {
	...
	function getAuto(){
		$proses = $this->db->query("SELECT max(auto) as auto from trkib_a_input")->row('auto');
	...
	function editData($post){
		...
		switch ($sts) {
			case 1:
				$query = "UPDATE trkib_a_input SET  
		...
		try{
			if(count($ex) > 0){
				foreach($ex as $idx=>$val){
					$query = "SELECT foto1,foto2,foto3,foto4,upload_sert as foto5 from trkib_a_input where id_lokasi='$val' and sts!='1'";
					...
					$query = "DELETE from trkib_a_input where id_lokasi='$val' and sts!='1'";
					$sql = $this->db->query($query);
				}
	...
	// no_reg,nm_barang,merek,tahun,harga,keterangn	
		$sql = "SELECT count(*) as tot FROM trkib_a_input a LEFT JOIN mbarang_new_modif b ON a.kd_brg=b.kd_brg
        ...
        $sql = "SELECT a.*,left(b.kd_brg,8) rincian_objek,left(b.kd_brg,14) sub_rincian_objek,
				...
				c.nm_skpd,b.uraian nm_brg,f.nm_lokasi, g.cara_peroleh
				FROM trkib_a_input a 
	...
	public function getDokumen($kib){
		...
		if($otori == '01'){
		$sql	= "SELECT a.*,c.tahun,left(b.kd_brg,11) rincian_objek,left(b.kd_brg,14) sub_rincian_objek,b.uraian nm_brg,c.kd_wilayah FROM transaksi.trd_isianbrg a join mbarang b 
	...
	public function getRincian($lccq,$akun,$kel,$jenis){
		...
		$sql	= "SELECT kd_brg,rincian_objek,uraian FROM mbarang 
...
/* 		$data = array();
	...
	function max_number($kd_unit,$kd_barang){
		$proses = $this->db->query("SELECT MAX(left(no_reg,6)) AS nomor FROM trkib_a_input where kd_unit='$kd_unit' and kd_brg='$kd_barang'")->row('nomor'); 
		if($proses=='null'){
			$hasil	= '0000001';
		}else{
