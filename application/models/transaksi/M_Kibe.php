...
class M_Kibe extends CI_Model {
	...
	function saveData($post){
			...
			$query = "INSERT INTO trkib_e_input(
...
// print_r($query); 
	...
	function getAuto(){
		$proses = $this->db->query("SELECT max(auto) as auto from trkib_e_input")->row('auto');
	...
	function editData($post){ 
		...
		switch ($sts) {
			case 1:
				$query = "UPDATE trkib_e_input SET  
					no_oleh         =  '$no_oleh',
					detail_brg      =  '$detail',
					tgl_oleh     	=  '$tgl_oleh',
					kondisi         =  '$kondisi',
	...
	function load_ruang($otori,$skpd) { 
		...
		$sql = "SELECT count(*) as tot FROM trkib_e_input a
		...
		try{
			if(count($ex) > 0){
				foreach($ex as $idx=>$val){
					$query = "SELECT foto1,foto2,foto3,foto4 from trkib_e_input where id_lokasi='$val' and sts!='1'";
					...
					$query = "DELETE from trkib_e_input where id_lokasi='$val' and sts!='1'";
					$sql = $this->db->query($query);
				}
	...
	public function loadHeader($key1,$key2,$key3){
		...
		if($otori=='01'){
			...
			$kondisi = "where a.kd_skpd='$skpd' and a.kd_unit='$unit'";
		}
		$xkey1 ='';
		$xkey2 ='';
		$xkey3 ='';
			...
			$sql = "SELECT count(*) as tot from trkib_e_input a
				LEFT JOIN mbarang_new_modif b ON a.kd_brg=b.kd_brg
				left join cara_peroleh g on a.asal=g.kd_cr_oleh  $kondisi" ;
 // print_r($sql); exit();
			$query1 = $this->db->query($sql);
			$total = $query1->row();
			
			$sql = "SELECT a.*,left(b.kd_brg,8) rincian_objek,left(b.kd_brg,14) sub_rincian_objek, b.uraian as nm_rincian, c.nm_skpd,b.uraian nm_brg,f.nm_lokasi,a.ket_matriks
				FROM trkib_e_input a 
				LEFT JOIN mbarang_new_modif b ON a.kd_brg=b.kd_brg
				LEFT JOIN ms_skpd c ON a.kd_skpd=c.kd_skpd 
				LEFT JOIN mlokasi f ON a.kd_unit = f.kd_lokasi and a.kd_skpd=f.kd_skpd
				left join cara_peroleh g on a.asal=g.kd_cr_oleh $kondisi
...
// print_r($sql);
	...
	public function getDokumen($kib){
		...
		$skpd = $this->session->userdata['skpd'];
		if($otori == '01'){
		$sql	= "SELECT a.*,c.tahun,left(b.kd_brg,11) rincian_objek,left(b.kd_brg,14) sub_rincian_objek,b.uraian nm_brg,kd_wilayah FROM transaksi.trd_isianbrg a join mbarang_new_modif b 
	...
	public function getRincian($lccq,$akun,$kel,$jenis){
		...
		$sql	= "SELECT kd_brg,left(kd_brg,8) rincian_objek,uraian FROM mbarang_new_modif 
	...
	public function getSubRincian($lccq,$akun,$kel,$jenis,$rincian){
		...
		$sql	= "SELECT kd_brg,sub_rincian_objek,uraian FROM mbarang_new_modif 
	...
	function prosesPindah($post){
		...
		try{
			if(count($ex) > 0){
				foreach($ex as $idx=>$val){ 
					$query = "update trkib_e_input set kd_ruang='$ruang' where id_lokasi='$val'";
					$sql = $this->db->query($query); 
				}
	...
	public function max_number($tahun,$unit,$kd_brg){
		...
		FROM trkib_e_input WHERE kd_unit='$unit' and kd_brg='$kd_brg'");  
