...
class M_Sewa extends CI_Model {
	...
	function simpan_header($post,$status,$no_dok,$skpd){
		...
		try {
			if($status!='detail'){
				$ck = $this->db->query("SELECT no_dokumen FROM transaksi.trh_sewa
                           WHERE no_dokumen = '$no_dok' and kd_uskpd='$skpd'");	
				if($ck->num_rows() == 0) {
					$this->db->insert('transaksi.trh_sewa', $post);
						return 1;
				} else {
						return 0;
				}
				...
				$del = $this->db->where('no_dokumen',$post['no_dokumen'])
							...
							->delete('transaksi.trh_sewa');
				if($del){
				$sql = $this->db->insert('transaksi.trh_sewa', $post);
				}
					if ($sql) {
						return 1;
						$sql->free_result();
			...
			if($status!='detail'){						
					foreach($post as $row) {							
							$filter_data = array(
								...
								"total" => str_replace(array(','), array(''), $row->biaya_sewa),
								"status" => '',
								"kd_rek" => '',
								"biaya_sewa" => str_replace(array(','), array(''), $row->biaya_sewa),
								"uraian_sewa" => str_replace(array(','), array(''), $row->uraian_sewa),
								"ket" => htmlspecialchars($row->uraian_sewa, ENT_QUOTES),
								"satuan" => htmlspecialchars($row->satuan, ENT_QUOTES),
								"kd_kegiatan" => htmlspecialchars($row->kd_kegiatan, ENT_QUOTES)
							);
						$sql = $this->db->insert('transaksi.trd_sewa', $filter_data);
						}
			}else{
				$del = $this->db->where('no_dokumen',$no_dokumen)
							->where('kd_uskpd',$kd_skpd)
							->delete('transaksi.trd_sewa');
				if($del){
						foreach($post as $row) {							
							$filter_data = array(
								"no_dokumen" => htmlspecialchars($no_dokumen, ENT_QUOTES),
	...
	public function load_trd($param)
	{
		...
		$csql = "SELECT SUM(total) AS total from transaksi.trd_sewa 
		where no_dokumen = '$nomor' and kd_uskpd = '$skpd'";
        $rs   = $this->db->query($csql)->row() ; 
		
        $sql = "SELECT b.* FROM transaksi.trh_sewa a 
				INNER JOIN transaksi.trd_sewa b ON a.no_dokumen=b.no_dokumen 
        ...
        {            
            $result[] = array(                                
                ...
                'total'         => $resulte['total'],
                'sum'         	=> $rs->total,                        
                'biaya_sewa'=> $resulte['biaya_sewa'],           
                'uraian_sewa'=> $resulte['uraian_sewa'],          
                'ket'           => $resulte['ket'],                        
	...
	public function getBarang($param)
	{
		...
		foreach ($query->result_array() as $key) {
			$res[] = array(
                ...
                'merek' 	=> $key['merek'],  
                'jumlah' 	=> $key['jumlah'],  
                'nilai' 	=> number_format($key['nilai'],2),  
                'total' 	=> number_format($key['jumlah'] * $key['nilai'],2),  
