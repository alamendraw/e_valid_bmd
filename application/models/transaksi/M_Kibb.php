 ...
 if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	public function loadHeader($key1,$key2,$key3,$otori,$skpd) { 
		...
		if($key3!=''){ //Sumber dana / merek / no polisi / kondisi / keterangan / nilai
			$xkey3 = "and (upper(g.cara_peroleh) like upper('%$key3%') or upper(a.merek) like upper('%$key3%') or upper(a.no_polisi) like upper('%$key3%') or upper(a.kondisi) like upper('%$key3%') or upper(a.keterangan) like upper('%$key3%') or a.nilai like '%$key3%')";	 
		}
		if($key1!='' || $key2!='' || $key3!=''){
...
// print_r($query); exit();
	...
	public function getDokumen($kib){
		...
		$skpd = $this->session->userdata['skpd'];
		if($otori == '01'){
		$sql	= "SELECT a.*,c.tahun,left(b.kd_brg,11) rincian_objek,left(b.kd_brg,14) sub_rincian_objek,b.uraian nm_brg, kd_wilayah FROM trd_isianbrg a join mbarang_new_modif b 
		on a.kd_brg=b.kd_brg 
		join trh_isianbrg c on a.kd_skpd=c.kd_skpd and a.no_dokumen=c.no_dokumen and a.id_lock=c.id_lock 
	...
	public function getRincian($lccq,$akun,$kel,$jenis){
		...
		$sql	= "SELECT kd_brg,rincian_objek,objek,uraian FROM mbarang_new_modif 
